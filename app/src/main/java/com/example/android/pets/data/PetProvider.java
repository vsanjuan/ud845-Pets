...
/**
 * Created by Salvador on 29/10/2016.
 */
...
public class PetProvider extends ContentProvider {
    ...
    private Uri insertPet(Uri uri, ContentValues values) {
        ...
        // Check the gender belongs to the defined values
        int gender = values.getAsInteger(PetEntry.COLUMN_PET_GENDER);
        if (gender != 0 && gender != 1 && gender != 2) {
            throw new IllegalArgumentException("Gender values must be 0, 1, or 2");
        }
    ...
    private int updatePet(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(PetEntry.COLUMN_PET_WEIGHT)) {
            int weight = values.getAsInteger(PetEntry.COLUMN_PET_WEIGHT);
            if (weight < 0) {
                throw new IllegalArgumentException("Weight must bue positive");
            }
        }
